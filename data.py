...
def cv_random_flip(img, label, mask, gray):
...
def randomCrop(image, label, mask, gray):
    border = 30
...
def randomRotation(image, label, mask, gray):
    mode = Image.BICUBIC
...
class SalObjDataset(data.Dataset):
    def w__init__(self, image_root, gt_root, mask_root, gray_root, trainsize):
    ...
    def __getitem__(self, index):
        ...
        # print('*****************************')
        image, gt, mask, gray = cv_random_flip(image, gt, mask, gray)
        image, gt, mask, gray = randomCrop(image, gt, mask, gray)
        # print('#############################')
        image, gt, mask, gray = randomRotation(image, gt, mask, gray)
        # print('-------------------------------')
...
def get_loader(image_root, gt_root, mask_root, gray_root, batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
    dataset = SalObjDataset(image_root, gt_root, mask_root, gray_root, trainsize)
