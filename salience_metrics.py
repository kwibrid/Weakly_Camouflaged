...
if __name__ == '__main__':
    gt_path = './dataset/gt/test_img/'
    # test_path = './results/'
    test_path = 'results/results_scribble/'
    ...
    for i_index,i in enumerate(os.listdir(test_path)):
        gt = cv2.imread(gt_path+i, 0)
        s_map = cv2.imread(test_path+i, 0)
        s_map_norm = normalize_map(s_map)
        print(i_index)
        auc_judd_score.append(auc_judd(s_map_norm, gt))
        # auc_borji_score.append(auc_borji(s_map_norm, gt))
        # print('auc borji :', auc_borji_score)
        # auc_shuff_score = auc_shuff(s_map_norm, gt, gt)
        # print('auc shuffled :', auc_shuff_score)
        nss_score.append(nss(s_map, gt))
        # print('nss :', nss_score)
        infogain_score.append(infogain(s_map_norm, gt, gt))
        # print('info gain :', infogain_score)
        # continous gts
        sim_score.append(similarity(s_map, gt))
        # print('sim score :', sim_score)
        cc_score .append(cc(s_map, gt))
        # print('cc score :', cc_score)
        kldiv_score.append(kldiv(s_map, gt))
        # print('kldiv score :', kldiv_score)
    mean_auc_judd_score = sum(auc_judd_score)/len(auc_judd_score)
    # mean_auc_borji_score = sum(auc_borji_score) / len(auc_borji_score)
    # mean_auc_shuff_score = sum(auc_shuff_score) / len(auc_shuff_score)
    mean_nss_score = sum(nss_score)/len(nss_score)
    mean_infogain_score = sum(infogain_score)/len(infogain_score)
    mean_sim_score = sum(sim_score)/len(sim_score)
    mean_cc_score = sum(cc_score)/len(cc_score)
    mean_kldiv_score = sum(kldiv_score)/len(kldiv_score)

    print('auc judd :', mean_auc_judd_score)
    # print('auc borji :', mean_auc_borji_score)
    # print('auc shuff :', mean_auc_shuff_score)
    print('nss :', mean_nss_score)
    print('info gain :', mean_infogain_score)
    print('sim score :', mean_sim_score)
    print('cc score :', mean_cc_score)
    print('kldiv score :', mean_kldiv_score)